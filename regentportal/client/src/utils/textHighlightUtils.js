...
// `normalizeHighlightsState` below is a behaviourally identical copy of
// server/utils/sanitizeHighlights.js, which is the real trust boundary. This
// copy exists because the browser normalizes the same data before painting it.
...
// out of the payload sent to the server.
...
const isValidHighlightRange = (range) =>
  !!range &&
  typeof range === 'object' &&
...
export const normalizeHighlightsState = (value) => {
  ...
  for (const [regionId, ranges] of Object.entries(value)) {
    ...
    for (const range of ranges.slice(0, MAX_HIGHLIGHT_RANGES_PER_REGION)) {
      if (!isValidHighlightRange(range)) continue;
